...
export async function initInjectionProjectTables(db: DatabaseLike) {
  await db.exec(`CREATE TABLE IF NOT EXISTS injection_projects (
    ...
    plan_status TEXT NOT NULL DEFAULT 'draft', lifecycle_status TEXT NOT NULL DEFAULT 'pending',
    unit TEXT, boiler TEXT, planned_start_date TEXT, planned_end_date TEXT, gas_support TEXT, schedule_status TEXT, source_import_id INTEGER,
  ...
  const existing = new Set((await db.all('PRAGMA table_info(injection_projects)')).map((column) => column.name));
  const additions = [
    ['unit', 'TEXT'], ['boiler', 'TEXT'], ['planned_start_date', 'TEXT'], ['planned_end_date', 'TEXT'],
    ['gas_support', 'TEXT'], ['schedule_status', 'TEXT'], ['source_import_id', 'INTEGER'],
  ];
  for (const [name, definition] of additions) {
    if (!existing.has(name)) await db.exec(`ALTER TABLE injection_projects ADD COLUMN ${name} ${definition}`);
  }
}
...
export async function createInjectionProject(db: DatabaseLike, input: ProjectInput): Promise<InjectionProject> {
  ...
  const result = await db.run(`INSERT INTO injection_projects (project_no, well_no, block, process_type, planned_steam, planned_pressure, planned_rate, planned_transfer_date, owner, remark, unit, boiler, planned_start_date, planned_end_date, gas_support, schedule_status, source_import_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [projectNo, input.wellNo.trim(), input.block.trim(), input.processType.trim(), input.plannedSteam ?? null, input.plannedPressure ?? null, input.plannedRate ?? null, input.plannedTransferDate, input.owner.trim(), input.remark || '', input.unit ?? null, input.boiler ?? null, input.plannedStartDate ?? null, input.plannedEndDate ?? null, input.gasSupport ?? null, input.scheduleStatus ?? null, input.sourceImportId ?? null, now, now]);
