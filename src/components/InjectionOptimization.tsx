...
import type { InjectionOperationOptimizerInput, InjectionOperationRecommendationResult } from '../lib/injectionOperationOptimizer';
...
export function InjectionOptimization() {
  ...
  const [optimizerInput, setOptimizerInput] = useState<InjectionOperationOptimizerInput | null>(null);
  useEffect(() => {
    ...
    void fetch(`/api/injection-operation-recommendations${query}`).then((response) => response.json()).then((payload) => { if (payload.success) { setRecommendations(payload.data); setOptimizerInput(payload.input); } });
  }, [block]);
  async function submitAdjustment(planId: string) {
    if (!optimizerInput || !adjustmentReason.trim()) return;
    const response = await fetch('/api/injection-operation-recommendations/evaluate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...optimizerInput, adjustments: [...(optimizerInput.adjustments ?? []), { planId, reason: adjustmentReason.trim(), patch: {} }] }) });
    const payload = await response.json();
    if (payload.success) { setRecommendations(payload.data); setOptimizerInput({ ...optimizerInput, adjustments: [...(optimizerInput.adjustments ?? []), { planId, reason: adjustmentReason.trim(), patch: {} }] }); setAdjustmentReason(''); } else setError(payload.message || 'Adjustment failed');
  }
