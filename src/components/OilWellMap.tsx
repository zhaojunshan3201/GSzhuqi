...
export function OilWellMap({ isAdmin, onNavigate }: OilWellMapProps) {
  ...
  return <div className="page-stack animate-in fade-in duration-300">
    <section className="app-card p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 rounded border border-slate-200 bg-slate-50 p-3 text-sm"><label className="flex items-center gap-2 font-bold"><input type="checkbox" checked={showRelations} onChange={(event) => setShowRelations(event.target.checked)} />???????{visibleRelations.length}?</label>{(['confirmed', 'suspected', 'released'] as ChannelingRelation['status'][]).map((status) => <label key={status} className="flex items-center gap-1"><input type="checkbox" checked={relationStatusFilters.includes(status)} onChange={(event) => setRelationStatusFilters((current) => event.target.checked ? [...current, status] : current.filter((item) => item !== status))} />{status === 'confirmed' ? '??' : status === 'suspected' ? '??' : '??'}</label>)}<select className="field-control w-auto" value={relationImpactFilter} onChange={(event) => setRelationImpactFilter(event.target.value as '' | ChannelingRelation['impactLevel'])}><option value="">??????</option><option value="high">???</option><option value="medium">???</option><option value="low">???</option></select></div>
