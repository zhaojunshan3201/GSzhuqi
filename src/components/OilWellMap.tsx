...
import type { ChannelingRelation } from '../lib/channelingProjectStore';
import { filterMapRelations, getMapRelationStyle } from '../lib/channelingMapRelations';
...
export function OilWellMap({ isAdmin, onNavigate }: OilWellMapProps) {
  ...
  const [relationStatusFilters, setRelationStatusFilters] = useState<ChannelingRelation['status'][]>(['confirmed', 'suspected']);
  const [relationImpactFilter, setRelationImpactFilter] = useState<'' | ChannelingRelation['impactLevel']>('');
  ...
  useEffect(() => {
    void fetch('/api/channeling-projects').then((response) => response.json()).then(async (payload) => {
      if (!payload.success) return;
      const batches = await Promise.all(payload.data.map((project: { id: number }) => fetch(`/api/channeling-projects/${project.id}/relations`).then((response) => response.json())));
      setRelations(batches.flatMap((item) => item.success ? item.data : []));
    }).catch(() => setMessage('关系数据读取失败'));
  }, []);
  ...
  const mapCoordinates = new Map(mapWells.filter((well) => well.xPercent != null && well.yPercent != null).map((well) => [well.wellNo, well]));
  const visibleRelations = filterMapRelations(relations, { statuses: relationStatusFilters, impactLevel: relationImpactFilter }).filter((relation) => mapCoordinates.has(relation.injectionWell) && mapCoordinates.has(relation.productionWell));
  const relationWithoutCoordinates = filterMapRelations(relations, { statuses: relationStatusFilters, impactLevel: relationImpactFilter }).filter((relation) => !mapCoordinates.has(relation.injectionWell) || !mapCoordinates.has(relation.productionWell));
  ...
  return <div className="page-stack animate-in fade-in duration-300">
    <section className="app-card p-5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3 rounded border border-slate-200 bg-slate-50 p-3 text-sm"><label className="flex items-center gap-2 font-bold"><input type="checkbox" checked={showRelations} onChange={(event) => setShowRelations(event.target.checked)} />关系图层（{visibleRelations.length}）</label>{(['confirmed', 'suspected', 'released'] as ChannelingRelation['status'][]).map((status) => <label key={status} className="flex items-center gap-1"><input type="checkbox" checked={relationStatusFilters.includes(status)} onChange={(event) => setRelationStatusFilters((current) => event.target.checked ? [...current, status] : current.filter((item) => item !== status))} />{status === 'confirmed' ? '已确认' : status === 'suspected' ? '疑似' : '已解除'}</label>)}<select className="field-control w-auto" value={relationImpactFilter} onChange={(event) => setRelationImpactFilter(event.target.value as '' | ChannelingRelation['impactLevel'])}><option value="">全部影响</option><option value="high">高影响</option><option value="medium">中影响</option><option value="low">低影响</option></select></div>
    ...
    <section className="app-card overflow-hidden">
      ...
      {!mapData && !loading && mapError ? null : <div ref={mapViewportRef} className="relative min-h-[70vh] overflow-hidden bg-slate-100" style={{ height: mapSize?.height }} onMouseMove={(event) => dragStart && setOffset({ x: event.clientX - dragStart.x, y: event.clientY - dragStart.y })} onMouseUp={() => setDragStart(null)} onMouseLeave={() => setDragStart(null)}>
        <div ref={mapRef} className={`absolute left-1/2 top-1/2 select-none ${calibrationMode && selectedWellNo ? 'cursor-crosshair' : 'cursor-grab'}`} style={{ width: mapSize?.width, height: mapSize?.height, opacity: mapSize ? 1 : 0, transform: `translate(calc(-50% + ${offset.x}px), calc(-50% + ${offset.y}px)) scale(${scale})` }} onMouseDown={(event) => { if (!calibrationMode) setDragStart({ x: event.clientX - offset.x, y: event.clientY - offset.y }); }} onClick={saveMarker}>
          ...
          {showRelations && <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible">{visibleRelations.map((relation) => { const injection = mapCoordinates.get(relation.injectionWell)!; const production = mapCoordinates.get(relation.productionWell)!; const style = getMapRelationStyle(relation); return <line key={`relation-${relation.id}`} x1={`${injection.xPercent}%`} y1={`${injection.yPercent}%`} x2={`${production.xPercent}%`} y2={`${production.yPercent}%`} stroke={style.stroke} strokeWidth={style.strokeWidth} strokeDasharray={style.strokeDasharray} />; })}</svg>}
      ...
      <div className="border-t border-slate-100 p-3"><div className="mb-3 rounded bg-slate-50 p-3 text-sm"><b>未定位关系（{relationWithoutCoordinates.length}）</b>{relationWithoutCoordinates.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{relationWithoutCoordinates.map((relation) => <span className="rounded border bg-white px-2 py-1" key={relation.id}>{relation.injectionWell} → {relation.productionWell}</span>)}</div>}</div><button className="flex items-center gap-2 text-sm font-bold text-slate-700" onClick={() => setShowUnlocated((value) => !value)}>无坐标井（{unlocatedWells.length}）{showUnlocated ? <ChevronUp size={16} /> : <ChevronDown size={16} />}</button>{showUnlocated && <div className="mt-3 rounded bg-slate-50 p-3 text-sm"><p className="mb-2 text-amber-800">无坐标井不会被绘制；管理员可使用上方标定模式补充位置。</p><div className="flex flex-wrap gap-2">{unlocatedWells.map((well) => <button key={well.wellNo} className="rounded border border-slate-200 bg-white px-2 py-1 hover:border-red-300" onClick={(event) => { selectedWellTriggerRef.current = event.currentTarget; setSelectedWell(well); }}>{well.wellNo}</button>)}</div></div>}</div>
    </section>
