...
export function InjectionProductionCockpit({ onNavigate }: {
  ...
  return <div className="page-stack">
    ...
    <section className="grid gap-3 md:grid-cols-4 xl:grid-cols-7">{labels.map(([key, label, unit]) => <div key={key} className="app-card min-w-0 p-4"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 truncate text-xl font-bold xl:text-2xl">{data.metrics[key] == null ? '数据待补全' : formatCockpitMetric(data.metrics[key], key)}{data.metrics[key] != null && <span className="ml-1 text-sm font-normal">{unit}</span>}</p></div>)}</section>
