  ...
  return <section aria-label="单井跟踪台账" className="grid gap-5 lg:grid-cols-[300px_1fr]">
    <aside className="space-y-4 rounded-xl border border-slate-200 bg-white p-4">
      ...
      {role === 'admin' && <form aria-label="新建或复用单井档案" onSubmit={createProfile} className="space-y-2 rounded bg-slate-50 p-3">
        ...
        <input aria-label="新建井号" name="wellNo" required value={createDraft.wellNo} onInput={(e) => { const wellNo = e.currentTarget.value; setCreateDraft((value) => ({ ...value, wellNo })); }} placeholder="井号" className="w-full rounded border p-2"/>
        <input aria-label="新建区块" name="block" value={createDraft.block} onInput={(e) => { const block = e.currentTarget.value; setCreateDraft((value) => ({ ...value, block })); }} placeholder="区块" className="w-full rounded border p-2"/>
        <input aria-label="新建负责人" name="owner" value={createDraft.owner} onInput={(e) => { const owner = e.currentTarget.value; setCreateDraft((value) => ({ ...value, owner })); }} placeholder="负责人" className="w-full rounded border p-2"/>
