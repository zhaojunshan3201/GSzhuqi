...
export function MeasureWellSelection() {
  ...
  return <div className="page-stack animate-in fade-in duration-300">
    <section className="app-card p-6">
      ...
      {sources.some((source) => (source.skippedRowCount ?? 0) > 0 || (source.errorMessages?.length ?? 0) > 0) && <div className="mt-3 rounded border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">{sources.map((source) => <div key={`status-${source.sourceType}`}>{(source.skippedRowCount ?? 0) > 0 && <span>{`${selectionSourceLabel(source.sourceType)}：跳过 ${source.skippedRowCount} 行`}</span>}{source.errorMessages?.length ? <span className="ml-3 text-red-700">{`导入错误：${source.errorMessages.map(formatSelectionImportError).join('；')}`}</span> : null}</div>)}</div>}
